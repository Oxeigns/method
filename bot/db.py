...
class Store:
    ...
    async def allowed(self, uid, sid):
        ...
        WHERE e.user_id=$1 AND e.service_id=$2 AND NOT u.is_banned AND NOT e.revoked
        AND s.is_active AND (e.expires_at IS NULL OR e.expires_at>now()))''', uid, sid)
    ...
    async def order(self, uid, sid, currency, terms_version):
        async with self.pool.acquire() as c, c.transaction():
            ...
            s = await c.fetchrow('SELECT * FROM services WHERE service_id=$1 AND is_active', sid)
    ...
    async def _grant(self, c, t):
        ...
        ON CONFLICT(user_id,service_id) DO UPDATE SET revoked=false, expires_at=
        CASE WHEN $3=0 OR (entitlements.expires_at IS NULL AND NOT entitlements.revoked) THEN NULL
    ...
    async def review(self, tid, approve, admin_id):
        async with self.pool.acquire() as c, c.transaction():
            ...
            if approve and not await c.fetchval('''SELECT EXISTS(SELECT 1 FROM services s JOIN vault_data v USING(service_id) WHERE s.service_id=$1 AND s.is_active AND v.is_published=1)''',t['service_id']):
    ...
    async def precheckout(self, uid, tid, currency, amount):
        ...
        AND t.status='Pending' AND t.expires_at>now() AND NOT u.is_banned AND s.is_active
